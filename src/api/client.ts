...
async function request<T>(
  ...
  // Add Django CSRF token if present
  ...
  const response = await fetch(`${API_BASE}${endpoint}`, {
    ...
    credentials: 'include', // Required for HttpOnly cookies (e.g. sessionid, csrftoken)
    headers,
  });

  if (!response.ok) {
    const error = await response.json().catch(() => ({}));
    const detail = error.error?.message || error.detail || error.message;
    let message: string;
  ...
  return (json && typeof json === 'object' && 'data' in json && !('items' in json && 'total' in json))
    ? json.data
    : json;
}
...
// System Notifications (từ backend Django)
...
export interface OperationScheduleRowData {
  id?: number;
  order?: number;
  time_start: string;
  time_end: string;
  units: Record<string, number | null | string>;
}
...
export interface OperationPlanData {
  ...
  station_id?: number;
  station_name?: string;
  scenario_type: 'recommended' | 'custom';
  scenario_type_display?: string;
  recalc_hours: number;
  inflow_mode: 'forecast' | 'manual';
  inflow_mode_display?: string;
  inflow_manual_m3s?: number | null;
  schedule_mode: 'fixed' | 'daily_cycle' | 'recommended';
  ...
  created_at?: string;
  updated_at?: string;
  schedule?: OperationScheduleRowData[];
}

export interface OperationPlansResponse {
  station: {
    id: number;
    name: string;
    plant_code?: string;
  ...
  active_plan: OperationPlanData | null;
  draft_plan: OperationPlanData | null;
  plans: OperationPlanData[];
}
...
export const api = {
  ...
  // Operation Plans
  getOperationPlans: () => request<OperationPlansResponse>('/operation-plans'),
  getOperationPlan: (id: number) => request<OperationPlanData>(`/operation-plans/${id}`),
  saveOperationPlan: (data: any) =>
    request<OperationPlanData>('/operation-plans', {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  applyOperationPlan: (id: number) =>
    request<OperationPlanData>(`/operation-plans/${id}/apply`, {
      method: 'POST',
    }),
  deleteOperationPlan: (id: number) =>
    request<{ deleted: boolean; id: number }>(`/operation-plans/${id}`, {
      method: 'DELETE',
    }),
  ...
  login: (data: LoginRequest) =>
    request<LoginResponse | UserResponse>('/auth/login', {
      method: 'POST',
      body: JSON.stringify({
  ...
  logout: () =>
    request<{ logged_out: boolean; message?: string }>('/auth/logout', {
      method: 'POST',
    }),
  getCurrentUser: () => request<UserResponse>('/me'),
  updateProfile: (data: {
    display_name?: string;
    phone?: string;
    organization?: string;
    ...
    request<UserResponse>('/me/profile', {
      method: 'PATCH',
      body: JSON.stringify(data),
    }),
  disableAuth: () =>
  ...
  // System Notifications (backend Django)
  getSystemNotifications: (params?: { page?: number; page_size?: number; unread_only?: boolean }) => {
    const qs = new URLSearchParams();
    if (params?.page) qs.set('page', String(params.page));
    if (params?.page_size) qs.set('page_size', String(params.page_size));
    if (params?.unread_only) qs.set('unread_only', '1');
    const query = qs.toString();
    return request<SystemNotificationsResponse>(`/notifications${query ? '?' + query : ''}`);
  },
  getSystemNotificationUnreadCount: () =>
    request<SystemNotificationUnreadResponse>('/notifications/unread-count'),
  markSystemNotificationRead: (id: number) =>
    request<{ notification: SystemNotification; unread_count: number }>(`/notifications/${id}/read`, { method: 'POST' }),
  markAllSystemNotificationsRead: () =>
    request<{ marked_count: number; unread_count: number }>('/notifications/read-all', { method: 'POST' }),
