...
file_path = '/mnt/d/job/thuydien/docker/frontend/src/components/StationWorkbenchDrawer.tsx'

with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Specific compound class replacements
replacements = [
    # Dark variants first
...
# 2. Fix inputs/selects that had text-white hardcoded next to bg-white
# When className has "bg-white dark:bg-zinc-950" and "text-white" -> change to "text-slate-900 dark:text-white"
content = re.sub(
    r'(className="[^"]*?bg-white dark:bg-zinc-950[^"]*?)\btext-white\b([^"]*?")',
    r'\1text-slate-900 dark:text-white\2',
    content
)

# 3. Fix modal header text-white
content = content.replace('text-sm font-bold text-white flex items-center gap-2', 'text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2')
content = content.replace('text-xs font-bold text-white', 'text-xs font-bold text-slate-900 dark:text-white')

# 4. Clean up any double dark: prefixes if created
content = content.replace('dark:dark:', 'dark:')

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)

print("Updated StationWorkbenchDrawer.tsx successfully!")
