...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const checkAuthStatus = async () => {
    try {
      ...
      try {
        ...
        if (mountedRef.current && currentUser && currentUser.id) {
          setUser(currentUser);
  ...
  const login = async (username: string, password: string, persistence: TokenPersistence = 'session'): Promise<LoginResponse> => {
    const response = await api.login({
      ...
      identifier: username,
      password,
      remember: persistence === 'persistent',
    });

    if ('id' in response && 'username' in response) {
      const userObj = response as unknown as UserResponse;
      setUser(userObj);
      setAuthEnabled(true);
      return { user: userObj, access_token: 'session_authenticated' };
    }

    const loginResp = response as LoginResponse;
    if (!loginResp.requires_2fa && loginResp.access_token) {
      setAuthToken(loginResp.access_token, persistence);
      if (loginResp.user) {
